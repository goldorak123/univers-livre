...
files.forEach(f => {
    ...
    if (content.includes(f.newBtn)) {
        content = content.replace(f.newBtn, f.oldBtn);
    } else {
        // Fallback pour les espaces/sauts de ligne
        const regex = new RegExp('<a href="extrait.*?class="book-cta-btn"[\\s\\S]*?<\\/a>');
        const match = content.match(regex);
        if (match) {
            content = content.replace(match[0], f.oldBtn);
        }
    }

    // Retirer le CSS
    if (content.includes(css)) {
        content = content.replace(css, '</head>\n');
    } else if (content.includes('/* BOOK CTA BUTTON */')) {
        const styleRegex = new RegExp('\n  \\/\\* BOOK CTA BUTTON \\*\\/[\\s\\S]*?<\\/style>\n', 'g');
        content = content.replace(styleRegex, '');
    }

    fs.writeFileSync(f.path, content);
    console.log('Reverted ' + f.path);
});
